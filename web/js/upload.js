...
class FileUploader {
    ...
    validateFile(file) {
        ...
        if (!isValidType && !isValidExtension) {
            this.showStatus('MP3またはMP4ファイルのみアップロード可能です', 'error');
            return false;
        }
    ...
    async performS3Upload(file) {
        ...
        // S3直接アップロード実行
        const uploadResult = await window.s3Client.uploadFileDirectly(file, onProgress);
        
        // Job IDから先頭3要素のみを抽出（Step Functionsと同じ形式）
        const cleanJobId = uploadResult.jobId.split('_').slice(0, 3).join('_');
        
        // DynamoDBにジョブレコード作成（短縮版Job IDを使用）
        ...
        // 処理状況一覧にジョブを追加（短縮版Job IDを使用）
        if (window.jobManager) {
            window.jobManager.addJob(file.name, cleanJobId);
        }
        
        // プログレス監視開始（短縮版Job IDを使用）
